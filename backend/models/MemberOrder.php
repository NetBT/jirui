...
class MemberOrder extends Common
{
    ...
    public function rules()
    {
        return [
            #添加
            [['member_id','combo_price','combo_id','number','gathering_fund','gathering_money','pay_type','price'], 'required','message' => '{attribute}不能为空','on' => 'add'],
            ...
            #编辑
            [['combo_id','number','gathering_fund','gathering_money','pay_type','price'], 'required','message' => '{attribute}不能为空','on' => 'edit'],
    ...
    {
        $newScenarios = [
            'add' => ['member_id','combo_id','combo_price','price','earnest','number','discount','integral','final_payment','pay_type','mark','gathering_fund','gathering_money','order_number'],
            'edit' => ['price','earnest','number','discount','integral','final_payment','combo_id','pay_type','order_type','mark','gathering_fund','gathering_money','order_number'],
        ];
...
//            $list['data'][$key]['order_type'] = Status::memberOrderTypeMap()[$value['order_type']];
            ...
            $list['data'][$key]['short_mark'] = (isset($value['mark']) && !empty($value['mark'])) ? Functions::chinese_str_cut(rtrim($value['mark'],','),50) : '--';
...
//            $list['data'][$key]['combo_id'] = $comboInfo[$value['combo_id']];
            ...
            if($value['order_type'] == Status::MEMBER_ORDER_COMBO_TYPE_NORMAL) {
                $info = MemberOrderCombo::getOneByWhere(['order_number' => $value['order_number']],'combo_order_number');
...
//            }
            ...
            if (!empty($id)) {
                ...
                $memberData['total_consume'] = bcadd($data['gathering_money'], $memberInfo['total_consume'], 4);//总消费是实收+原先的消费
                ...
                $comboArray = Combo::getByWhere(['is_delete' => Status::COMBO_NOT_DELETE,'business_id' => Common::getBusinessId()],['id','goods_content','combo_name','combo_type','combo_price','combo_integral','combo_discount','goods_content','combo_content']);
                $comboInfo = Functions::extractKey($comboArray,'id');

                $goodsArray = AbGoods::getByWhere(['AB_id' => Common::getBusinessId()],['id','goods_p','goods_code','goods_name','goods_category','goods_price','goods_cost','goods_discount','goods_color','goods_size','goods_texture','goods_style','goods_type']);
                ...
                $res = ABStatement::recordStatement(Status::AB_STATEMENT_TYPE_ORDER_FIRST_PAY,$post['gathering_money'],$data['order_number'].'的订单首付');
                if ($res !== true) {
    ...
    {
        //获取订单信息
        $memberOrderInfo = self::getOneInfoById($id,'order_number,member_id,price,number,id,member_id,total_money,final_payment,earnest');
    ...
    {
        //获取订单信息
        $memberOrderInfo = self::getOneInfoByOrderId($order_number,'order_number,member_id,price,number,id,member_id,total_money,final_payment,earnest');
        $memberOrderInfo['price'] = floatval($memberOrderInfo['price']);
        $memberOrderInfo['total_money'] = floatval($memberOrderInfo['total_money']);
        $memberOrderInfo['final_payment'] = floatval($memberOrderInfo['final_payment']);
        $memberOrderInfo['earnest'] = floatval($memberOrderInfo['earnest']);
    ...
    public function addShop()
    {
        ...
        $goodsInfo = AbGoods::getByWhere(['goods_code' => $post['goods_code']],['id','goods_p','goods_code','goods_name','goods_category','goods_price','goods_cost','goods_discount','goods_color','goods_size','goods_texture','goods_style','goods_type']);
        //添加订单order_detail表  member_order_detail 添加详细的商品信息
...
//                }
    ...
    public function refund()
    {
        ...
        try {
            ...
            $res = ABStatement::recordStatement(Status::AB_STATEMENT_TYPE_ORDER_REFUND,$post['refund_money'],$memberOrderInfo['order_number'].'的订单退款');
            if ($res !== true) {
                throw new Exception('添加流水失败');
            }
    ...
    public function secondGathering()
    {
        ...
        try {
            ...
            $res = ABStatement::recordStatement(Status::AB_STATEMENT_TYPE_SECOND_SALE,$post['second_money'],$orderInfo['order_number'].'的订单二销收款');
            if ($res !== true) {
                throw new Exception('添加流水失败');
            }
            $trans->commit();
    ...
    public function quoteOrderInfo() {
        try {
            ...
            $comboOrderInfo = MemberOrderCombo::getByWhere($where, ['combo_name', 'price', 'order_number','combo_order_number', 'combo_id']);
