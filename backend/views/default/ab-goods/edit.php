...
<article class="page-container col-xs-12">
    <?php $form = \yii\bootstrap\ActiveForm::begin([
            ...
            'validationUrl' => \yii\helpers\Url::to(['ab-goods/validate-form', 'type' => 'edit']),
        ]
    );
    ?>
    <?= $form->field($model,'id',[
    ...
    <div class="col-xs-6 col-sm-6 cl text-center">
        <?= $form->field($model,'goods_color', [
            ...
            'options' => ['class' => '']
        ])->inline()->radioList(\common\models\Status::getABGoodsDefaultColor($id), [
    ...
    <div class="col-xs-6 col-sm-6 cl text-center">
        ...
        ])->inline()->radioList(\common\models\Status::getABGoodsDefaultSize($id), [
    ...
    <div class="col-xs-6 col-sm-6 cl text-center">
        <?= $form->field($model,'goods_type', [
            ...
            'options' => ['class' => '']
        ])->inline()->radioList(\common\models\Status::getABGoodsTypeMap(), [
            'template' => '{label}<div class="formControls col-xs-5 text-left" id="colorList">{input}</div>',
            'value' => \common\models\Status::AB_GOODS_TYPE_COMMON])?>
    </div>

    <div class="col-xs-6 col-sm-6 cl text-center">
        <?= $form->field($model,'goods_p',[
            'template' => '{label} <div class="formControls col-xs-7">{input}{error}</div>',
            'errorOptions' => [
                'tag' => 'label',
                'class' => 'error'
        ...
        ])->textInput(['placeholder'=>"填写P数", 'class' => 'input-text form-control '])->label('P数')?>
    </div>
    ...
    <div class="col-xs-12">
        ...
        <div class="col-xs-7" style="width: 80%">
            <div class="album-item" style="width: 200px;">
                <div class="album-img" id="album_cover" onclick="goods_image_add('<?= $id?>');">
                    <?php if(!empty($poster)) :?>
                    <img src="/uploads/<?= $poster?>" alt="..." class="radius">
                        <?php else:?>
    ...
    <div class="col-xs-12 col-sm-12 cl text-center">
        <button type="button" onclick="edit()" class='btn btn-hot btn-md margin-right-30'>保存并入库</button>
...
<script>
    ...
    function addGoodsColor() {
        layer_show({}, '添加商品颜色', '<?= \yii\helpers\Url::to(['ab-goods/add-color'])?>', 300, 200);
    }
    function addGoodsSize() {
        layer_show({}, '添加商品尺寸', '<?= \yii\helpers\Url::to(['ab-goods/add-size'])?>', 300, 200);
    }

    function goods_image_add(id){
        layer_show({id:id}, '', '<?= \yii\helpers\Url::to(['ab-goods-images/edit'])?>');
    }

    function edit() {
        ajaxSubmit('<?= \yii\helpers\Url::to(['ab-goods/do-edit'])?>',$('#editGoods').serialize(), function () {
            layer_close();
