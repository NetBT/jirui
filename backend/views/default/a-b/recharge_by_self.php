<article class="page-container">
    <form id="recharge" class="form form-horizontal" action="" method="post">
        ...
        <div class="col-xs-12 col-sm-12 cl">
            <label class="form-label col-xs-4 text-right">充值金额(元)</label>
        ...
        <div class="col-xs-12 col-sm-12 cl">
            ...
            <div class="form-group formControls col-xs-5 text-left">
                <label class="radio-inline"><input type="radio" name="type" checked value="<?= \common\models\Status::AB_RECHARGE_PAY_TYPE_WECHAT?>">微信</label>
                <label class="radio-inline"><input type="radio" name="type" value="<?= \common\models\Status::AB_RECHARGE_PAY_TYPE_ALIPAY?>">支付宝</label>
                <label class="error"></label>
            </div>
        </div>


    <div class="col-xs-12 col-sm-12 cl pay_type" id="pay_type_<?= \common\models\Status::AB_RECHARGE_PAY_TYPE_WECHAT?>">
    ...
    <div class="col-xs-12 col-sm-12 cl pay_type" id="pay_type_<?= \common\models\Status::AB_RECHARGE_PAY_TYPE_ALIPAY?>" style="display: none">
...
<script>
    ...
    function recharge() {
        ...
        if ($(".has-error", '#recharge').length)
        {
            return false;
        }

        layer.confirm('确认是否扫描二维码？', {icon: 0, title: '提示'}, function (index) {
            ajaxSubmitForm('#recharge', '<?= \yii\helpers\Url::to(['a-b/recharge-by-self'])?>');
