...
//                'labelOptions' => [],//改变label
            ...
            'validationUrl' => \yii\helpers\Url::to(['member-order/validate-form', 'type' => 'add']),
    ...
    <?= $form->field($model,'member_id',[
        'template' => '{input}',
        'options' => ['class' => '',]
    ])->hiddenInput(['value' => $memberInfo['id']])?>
    ...
    <div class="vip">
        <div class="vip_top clear-margin-bottom">
            ...
            <div class="vip_box_input" style="padding-top:10px">
                <a class="box">筛选条件</a>
                <input type="text" id="memberName" style="width: 178px;"
                    <?php if(isset($memberInfo) && !empty($memberInfo)) : ?>
                        placeholder="<?= $memberInfo['tel']?>"
                    <?php else:?>
                        placeholder="输入要搜索的会员手机号"
                    <?php endif;?>
                >
                ...
                <div class="text-right">
                    <p class="right text-hot clear-padding clear-margin" style="float: right">新用户，点击<button type="button" class="btn btn-warning size-MINI" onclick="addMember();">创建会员</button></p>
                </div>
        ...
        <?php if(\common\models\Functions::getABCommonByKey('order_date_on_off') == 1) :?>
            <div class="col-xs-6 col-sm-3 cl">
                <div class="form-group">
                    <lable class="form-label text-center" style="font-weight: bold">订单日期：<?= date('Y-m-d')?></lable>
                </div>
            </div>
        <?php endif;?>
        <?php if(\common\models\Functions::getABCommonByKey('order_number_on_off') == 1) :?>
            <?php if(\common\models\Functions::getABCommonByKey('member_order_coded_system') == 1) :?>
                <div class="col-xs-6 col-sm-4 cl text-center">
                    ...
                    ])->textInput(['placeholder'=>"订单编号",'value' => \backend\models\MemberOrder::getMemberOrderNum(),'readonly' => ''])?>
                </div>
            <?php else:?>
                <div class="col-xs-6 col-sm-4 cl text-center">
                    <?= $form->field($model,'order_number',[
                        ...
                        'template' => '{label} <div class="formControls col-xs-7">{input}{error}</div>',
                    ])->textInput(['placeholder'=>"订单编号",'value' => ''])?>
                </div>
            <?php endif;?>
        <?php endif;?>
        <div class="clear"></div>
            ...
            <table class="top" width="100%" border="0" cellspacing="0" cellpadding="0" id="memberTable">
                ...
                <?php if(isset($memberInfo) && !empty($memberInfo)) : ?>
                    <tr>
                        ...
                        <td><?= $memberInfo['parents_name']?></td>
                        <td><?= $memberInfo['parents_baby_link']?></td>
                        <td><?= $memberInfo['wechat']?></td>
                        <td><?= $memberInfo['valid_money']?></td>
                        <td><?= $memberInfo['integral']?></td>
                        <td><?= $memberInfo['total_consume']?></td>
                        <td><?= $memberInfo['spare_tel']?></td>
                    </tr>
                <?php endif;?>
                </tbody>
            </table>
    ...
    <div class="vip">
        ...
        <?php if(\common\models\Functions::getABCommonByKey('order_num_on_off') == 1) :?>
            ...
            <input id="number" class="number-input" name="MemberOrder[number]" value="1" onchange="changeNumber()" placeholder="数量" type="text">
        ...
        <?php if(\common\models\Functions::getABCommonByKey('order_earnest_on_off') == 1) :?>
            <div class="col-xs-6 col-sm-6 cl text-center">
                <?= $form->field($model,'earnest',[
                    'template' => '{label} <div class="formControls col-xs-7">{input}{error}</div>',
                ])->textInput(['placeholder'=>"定金",'value' => 0,'onchange' => 'changeEarnest()'])?>
            </div>
        <?php endif;?>
...
//                'options' => ['successCssClass' => ''],
    ...
    <div class="col-xs-12 col-sm-12 cl text-center margin-top-20 margin-bottom-10">
        <button type="button" onclick="add('add')" class='btn btn-hot btn-md margin-right-30'>创建订单</button>
        <button type="button" onclick="add('gua')" class='btn btn-hot btn-md margin-right-30'>挂单</button>
        ...
        $.ajax({
            ...
            success: function(data)
            {
                if(data.code == 1000) {
                    ...
                    var detailcombo = data.data.detailcombo;
...
//                    $('#memberorder-discount').val();
                    ...
                    var comoboTableHtml = '';
                    $.each(detailcombo,function(k,v){
                        comoboTableHtml += '<tr>';
                        <?php if(\common\models\Functions::getABCommonByKey('combo_name_on_off') == 1) :?>
                        comoboTableHtml += '<td>'+v.combo_name+'</td>';
                        <?php endif;?>
                        <?php if(\common\models\Functions::getABCommonByKey('combo_price_on_off') == 1) :?>
                        comoboTableHtml += '<td>'+v.combo_price+'</td>';
                        <?php endif;?>
                        <?php if(\common\models\Functions::getABCommonByKey('combo_volume_on_off') == 1) :?>
                        comoboTableHtml += '<td>'+v.register_count+'</td>';
                        <?php endif;?>
                        <?php if(\common\models\Functions::getABCommonByKey('combo_clothing_on_off') == 1) :?>
                        comoboTableHtml += '<td>'+v.combo_clothing+'</td>';
                        <?php endif;?>
                        <?php ?>
                        comoboTableHtml += '<td onclick="showdetails(\''+v.goods_content+'\')"><a>商品详情</a></td>';
                        <?php ?>
                        comoboTableHtml += '</tr>';
                    });
    ...
    function showdetails(goods_content) {
        $.ajax({
            url : '<?= \yii\helpers\Url::to(['combo/get-goods-info'])?>',
            type : 'POST',
            async: false,
...
//        $('#memberSimpleInfo').append(pHtml);
    ...
    function addMember()
    {
//        layer_show({}, '添加会员', '<?//= \yii\helpers\Url::to(['member/order-add']) ?>//',860,500);
        layer_show_full('添加会员', '<?= \yii\helpers\Url::to(['member/order-add']) ?>');
    }

    function add(type) {
        if (type == 'gua') {
            $('#addMemberOrder').prepend('<input type="hidden" value="2" name="orderstate"/>')
        } else {
