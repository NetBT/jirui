...
//                'labelOptions' => [],//改变label
            ...
            'validationUrl' => \yii\helpers\Url::to(['combo/validate-form', 'type' => 'growCombo']),
        ]
    );
    ?>
    <?= $form->field($model,'id',[
 ...
 <label class="notice text-left">注意:95折请填写0.95；如果填写多个折扣，请用英文逗号隔开，如：0.95,0.85,0.8。若不想添加折扣，请留空。</label></div>',
    ...
    <div class="col-xs-12 col-sm-12 cl text-center">
        <div class="form-group ">
            <label class="form-label col-xs-2 text-right">选择套系</label>
            <div class="fromControls col-xs-9 text-l"  id="">
                <button type="button" onclick="comboDetail()" class='btn btn-secondary btn-sm'>选择套系</button>
            </div>
        </div>
    </div>

    <div class="col-xs-12 col-sm-12 cl text-center">
        <?php $str = '';if($comboList):?>
            <?php foreach ($comboList as $key => $value): ?>
                <?php $str .= '<label class="checkbox-inline"><input type="checkbox" value="'.$key.'" class="disabled" name="Combo[combo_content][]" checked >'.$value.'</label>'?>
            <?php endforeach;?>
        <?php endif;?>
        <?= $form->field($model,'combo_content',[
            'template' => '{label} <div class=" formControls col-xs-9 text-left" id="comboContent">'.$str.'</div>',
        ])?>
    </div>
...
<script>
    function comboDetail(){
        layer_show({}, '', '<?= \yii\helpers\Url::to(['combo/combo-detail'])?>', 1000,'','');
    }

    function add() {
        DataTable.id = '#growComboList';
        ajaxSubmitForm('#generalAddEdit', '<?= \yii\helpers\Url::to(['combo/grow-add-edit'])?>');
    }
