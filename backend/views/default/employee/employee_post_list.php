<div class="main-page col-xs-12">
    <div class="main-header col-xs-12">
        ...
        <div class="header-search col-xs-8">
            <div class="col-xs-3 pull-right text-r">
                <button class="btn btn-yellow" onclick="employeePostList.postAddModel();">新加职位</button>
    ...
    <div class="main-body col-xs-12">
        <table class="tab" width="100%" border="0" cellspacing="0" cellpadding="0" id="employeePostList"></table>
    </div>
</div>
<script>
    $(function(){
        DataTable.init('initEmployeePostList','#employeePostList','<?= \yii\helpers\Url::to(['employee/list-employee-post'])?>',{});
        ...
        $("#doRefresh").bind('click',function(){
            DataTable.id = '#employeePostList';
            DataTable.drawTable();
        });
    });

    var employeePostList = {
        postUpdateStatusUrl : '<?= \yii\helpers\Url::to(['employee/post-update-status'])?>',
        postAddEditUrl : '<?= \yii\helpers\Url::to(['employee/post-add-edit'])?>',
